  ...
  return (
    ...
    >
      <SimpleBar onClick={closeSidebar} style={{ maxHeight: "100%" }}>
        ...
        <hr className="mt-2" />
        <div onClick={(e) => e.stopPropagation()} className="w-full h-[43vh] overflow-y-auto">
  {cartItems.length === 0 ? (
    <p>Your cart is empty!</p> 
  ) : (
    cartItems.map((item, index) => (
      <div key={index} className="w-full border h-[18vh] flex items-center mb-2">
        <div className="ml-2 w-[30%] h-[60%] border">
          <img src={`http://192.168.100.106:4000${item.imageUrls[0]}`} alt={item.name} className="w-full h-full object-cover" />
        </div>
        <div className="w-[100%] px-3">
          <h5 className="text-[16px] text-[#333333]">{item.name}</h5>
          <p className="text-[16px] text-[#333333]">${item.price}</p>
        </div>
        <div className="w-full sm:w-[30%] mr-2 mt-5">
          <div className="p-2">
            ...
            <div className="flex justify-center mt-2">
              <span
                className="px-2 border text-center cursor-pointer"
                // onClick={() => updateQuantity(item, 'decrement')} 
              >
                <Icon icon="line-md:minus" className="text-lg text-[#666666]" />
              </span>
              <span
                className="px-2 border text-center cursor-pointer"
                // onClick={() => updateQuantity(item, 'increment')} // Adjust quantity
              >
                <Icon icon="line-md:plus" className="text-lg text-[#666666]" />
              </span>
            </div>
          </div>
          <button
        ...
        <div className="w-full bg-gray-100 justify-center flex overflow-x-auto" onClick={(e) => e.stopPropagation()}>
      <span
      ...
      >
        <Icon icon="ic:outline-info" className={`text-[19px] ${infoOpen ? "text-[#5EC1A1]" : "text-[#333333]"}`} />
      </span>
      <span
      ...
      >
        <Icon icon="tabler:message" className={`text-[19px] ${msgOpen ? "text-[#5EC1A1]" : "text-[#333333]"}`} />
      </span>
      <span
        className={`border px-8 py-2 group cursor-pointer ${
          shippingOpen ? "border-none text-[#5EC1A1]" : "hover:text-[#5EC1A1]"
        }`}
        onClick={() => handleSectionToggle(setShippingOpen)}
      >
        <Icon icon="mingcute:truck-line" className={`text-[19px] ${shippingOpen ? "text-[#5EC1A1]" : "text-[#333333]"}`} />
      </span>
      <span
        className={`border px-8 py-2 group cursor-pointer ${
          couponCodeOpen ? "border-none text-[#5EC1A1]" : "hover:text-[#5EC1A1]"
        }`}
        onClick={() => handleSectionToggle(setCouponCodeOpen)}
      >
        <Icon icon="fluent:tag-16-regular" className={`text-[19px] ${couponCodeOpen ? "text-[#5EC1A1]" : "text-[#333333]"}`} />
      </span>
</div>
      ...
      {infoOpen && (
        ...
        >
          ...
          <div className="flex justify-between items-center w-full mt-4">
            ...
            <h4 className="text-[#222] font-Poppins text-right mr-4">$853.00</h4>
          </div>
          <h4 className="font-Poppins text-[#666666] text-[14px] ml-4 py-3">Taxes calculated at checkout</h4>
...
>
  <div className="w-full flex flex-col  justify-center items-center">
    <p  className="text-[#666666] text-[14px] font-Poppins text-center py-4">Special instructions for seller</p>
    <textarea className="w-[90%] border-2  resize-none outline-none py-2 px-2 text-[#666666] font-Poppins" cols="40" rows="3"></textarea>
  </div>
  <div className="flex justify-center mt-4">
  <button className="bg-[#5EC1A1] px-12 py-1 text-white resize-none text-[15px] outline-none hover:bg-[#5bb396] tracking-widest">Submit</button>
  </div>
</div>
 ...
 onClick={(e)=>e.stopPropagation()}
      ...
      <Listbox value={selectedCountry} onChange={handleCountryChange}>
        ...
        <div className="mt-4">
          <label className="block text-[#666666] text-sm mb-1">Postal Code</label>
          <div className="flex items-center justify-center gap-2">
            ...
            />
            <button
              className="bg-[#5EC1A1] py-2 px-4 text-white"
            >
              Calculate
            </button>
          </div>
 ...
 onClick={(e)=>e.stopPropagation()}
        className={`transition-transform transform ${couponCodeOpen ? 'translate-y-0' : 'translate-y-full'} px-4 duration-500 ease-out`}>
          ...
          <input type="text" placeholder="Your Code here" className="w-full border-2  text-[#666666] border py-1 px-2 outline-none"/>
       <p className="text-[#666666] font-Poppins text-[13px] py-2">Coupon code calculated at checkout
       <button className="w-full bg-[#5EC1A1] text-[14px] tracking-widest	  py-2 text-center  text-white resize-none mt-2 outline-none hover:bg-[#5bb396]">Submit</button>
</p>
        </div>
      )}
